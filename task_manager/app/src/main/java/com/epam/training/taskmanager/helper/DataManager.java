...
public class DataManager {
    ...
    loadData(
        ...
        if (IS_ASYNC_TASK) {
            executeInAsyncTask(callback, params, dataSource, processor, threadPoolName);
        } else {
            executeInThread(callback, params, dataSource, processor);
        }
    }

    private static <ProcessingResult, DataSourceResult, Params> void executeInAsyncTask(final Callback<ProcessingResult> callback,
                                                                                        Params params,
                                                                                        final DataSource<DataSourceResult, Params> dataSource,
                                                                                        final Processor<ProcessingResult, DataSourceResult> processor,
        ...
        new AsyncTask<Params, Void, ProcessingResult>(threadPoolName) {
